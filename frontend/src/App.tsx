...
function SharePage() {
  ...
  async function start() {
    if (!api) return;
    try {
      const next = await api.StartSharing({ codexDir: dir, port: 28888, shareConfig: selected.config, shareAuth: selected.auth, shareSkills: selected.skills });
      setStatus(next);
...
function SyncPage() {
  ...
  async function syncNow() {
    if (!api) return;
    try {
      const next = await api.SyncFromPeer({ peerIP, port: 28888, targetDir, syncConfig: selected.config, syncAuth: selected.auth, syncSkills: selected.skills });
      setResult(next);
